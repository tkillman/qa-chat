...
관리자가 "목록 조회" 탭에서 각 Q&A 항목 오른쪽에 삭제 버튼(🗑️)이 표시되어야 합니다.

**Why this priority**: 삭제 기능의 기본 진입점이며, 버튼이 없으면 기능을 사용할 수 없습니다.

**Independent Test**: 관리자 로그인 → "목록 조회" 탭 진입 → 각 카드에 삭제 버튼 확인
...
- **FR-001**: 관리자 목록 조회 탭의 각 Q&A 항목 카드에 삭제 버튼을 표시해야 함
...
- Q: 삭제 버튼 UI 배치 위치는? → A: 카드의 title/header 우측에 embedded (호버 불필요, 항상 표시)
